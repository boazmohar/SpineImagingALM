...
% appliquer 'axis image' 
% note that info.IsImg can be true from a previous display but the type of
% the new display is not an image -> do axis normal then
if ~isempty(info) && info.IsImg && strcmp(type,'image')
...
if isempty(hlist)
    ...
    if isimg
        if ~isfield(info1,'ImPoint')
            warning('there is a bug here, should be fixed')
            newpoint = [];
        else
...
elseif strcmp(get(gcf,'SelectionType'),'extend')    % zoom out (2x)
    ...
    if ~isimg, axis(hObject,newax), end
    for ha = hlist, chgAxis(ha,newax), end
elseif all(rectpixsize>2)                             % zoom to selection

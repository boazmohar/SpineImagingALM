    ...
    methods
        ...
        function getframe(M)
            ...
            if ~isempty(M.opt.xhigh) && ~isempty(M.opt.xlow)
                M.fr = filt2(M.fr,M.opt.xlow,'bzm',M.opt.xhigh); % keep mean
            elseif ~isempty(M.opt.xhigh)
                M.fr = filt2(M.fr,M.opt.xhigh,'hzm'); % keep mean
            elseif ~isempty(M.opt.xlow)
                M.fr = filt2(M.fr,M.opt.xlow,'lm');
            end
        ...
        function savemovie(M)
            ...
            if ~fname, return, end
            pixelsize = fn_input('pixelsize',4,1,10);
            ...
            disp('save avi')
            movie2avi(mov,fname,'fps',M.pars.fps);

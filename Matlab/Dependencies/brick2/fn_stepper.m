classdef fn_stepper < hgsetget
    ...
    methods
        function P = fn_stepper(varargin)
            ...
            P.hpanel = uipanel('parent',P.parent, ...
                ...
                'bordertype','none');
            addlistener(P.hpanel,'Position','PostSet',@(u,e)updateinternposition(P));
    ...
    methods
        ...
        function updateinternposition(P)
            unit = get(P.hpanel,'units');
            set(P.hpanel,'units','pixel');
            pospix = get(P.hpanel,'position');
            set(P.hpanel,'units',unit);
            w = pospix(3)/P.nx;
            h = pospix(4);
            w1 = min(h*2/3,w/2); %#ok<CPROP>
            w2 = w-w1;
            for k=1:P.nx
                set(P.hslider(k),'position', [1+(k-1)*w    1 w1+1 h])
                set(P.htext(k),  'position', [1+(k-1)*w+w1 1 w2 h])
            end
        end
    end

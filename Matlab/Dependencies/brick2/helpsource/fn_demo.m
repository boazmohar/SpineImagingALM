...
fn_errorbar(t,data,'facecolor',[.5 .5 1],'bar')
...
% values in |X| changes the control displays. Here is a simple usage
% example (<matlab:fn_control('demo') execute>). See |fn_movie| function
% for a more elaborate usage.

%%
%   s = struct('a',false,'b',1,'c',2,'d','hello');
%   spec = struct('c','xslider 0 10 1','d',{{'hello','yo'}});
%   myfun = @disp;
%   X = fn_control(s,spec);
...
%% GUI programs
%
% *|<help_fn_imvalue.html fn_imvalue>|*, when executed, automatically link together the image and
...
% same dimensions.
% 
% Try the following lines to get an idea (<matlab:fn_imvalue('demo')
% execute>).

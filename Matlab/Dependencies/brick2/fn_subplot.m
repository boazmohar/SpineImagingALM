...
% function ha = fn_subplot(hf,nrow,ncol,k)
...
if length(varargin)==4
    [hf nrow ncol k] = deal(varargin{:});
else
    [hf ngraph k] = deal(varargin{:});
...
if info.axes(k)
    ...
    pos = [(icol-1)/ncol (nrow-irow)/nrow 1/ncol 1/nrow];
    ha = axes('parent',hf,'units','normalized','pos',pos);
    info.axes(k) = ha;
end

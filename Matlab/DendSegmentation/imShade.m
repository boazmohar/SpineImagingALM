...
shadeimg(:,:,2) = shadeimg(:,:,2) .* (1-.75*double(mask1));  % delete channel

if nargin > 2
    shadeimg(:,:,3) = shadeimg(:,:,3) .* (1-.75*double(mask2));  % delete channel
end
   
if nargin > 3
    shadeimg(:,:,2) = shadeimg(:,:,2) .* (1-.25*double(mask3));  % delete channel
end
